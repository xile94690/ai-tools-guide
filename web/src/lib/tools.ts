...
export type RegionFilter = "all" | "domestic" | "global";

export function toolsByRegionFilter(filter: RegionFilter): Tool[] {
  if (filter === "all") return featuredTools;
  if (filter === "domestic") {
    return tools.filter((tool) => tool.region !== "global");
  }
  return tools.filter((tool) => tool.region !== "domestic");
}
